      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-2xl animate-[fadeIn_0.2s_ease-out]">
          <div className="w-full max-w-lg border border-[#333333] bg-[#131313] p-5 md:p-8 relative overflow-hidden shadow-[0_0_50px_rgba(255,255,255,0.05)]">
            ...
            {modalStep === "name" && (
              <form onSubmit={handleStep1} className="space-y-4 md:space-y-5">
                <h2 className="text-base md:text-xl font-bold text-white uppercase tracking-widest font-mono">
                  {rsvpType === "NO" ? "WHO IS BREAKING OUR HEARTS?" : "WHO'S MAKING THIS DECISION?"}
                </h2>
                <input
                  type="text"
                  required
                  autoFocus
                  value={guestName}
                  onFocus={() => soundEngine?.duckMusic(0.35, 0.3)}
                  onBlur={() => soundEngine?.unduckMusic(0.4)}
                  onChange={(e) => setGuestName(e.target.value)}
                  placeholder="[ YOUR NAME ]"
                  className="w-full bg-[#0e0e0e] border border-[#333333] p-3.5 font-mono text-white focus:outline-none focus:border-white transition-all uppercase text-xs md:text-sm placeholder:text-[#8e9192]"
                />
                <button
                  type="submit"
                  className="w-full border border-white bg-transparent py-3.5 font-mono uppercase tracking-widest hover:bg-white hover:text-black transition-all text-xs md:text-sm font-bold cursor-pointer"
                >
                  NEXT_STEP &gt;
                </button>
              </form>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button
                      key={n}
                      type="button"
                      onClick={() => handleSelectCount(String(n))}
                      className="border border-[#333333] bg-[#0e0e0e] py-3 font-mono text-white hover:bg-white hover:text-black transition-colors uppercase tracking-widest text-[11px] md:text-xs font-bold cursor-pointer"
                    >
                      {n} {n === 1 ? "PERSON" : "PEOPLE"}
                    </button>
                  ))}
                  <button
                    type="button"
                    onClick={() => handleSelectCount("more")}
                    className="border border-[#333333] bg-[#0e0e0e] py-3 font-mono text-white hover:bg-white hover:text-black transition-colors uppercase tracking-widest text-[11px] md:text-xs font-bold cursor-pointer"
                  >
                    MORE THAN 5
                  </button>
            ...
            {/* Step 3: Custom Count Input (Audio Ducking on Focus) */}
            {modalStep === "custom_count" && (
              <form onSubmit={handleCustomCountSubmit} className="space-y-4 md:space-y-5">
                <h2 className="text-base md:text-xl font-bold text-white uppercase tracking-widest font-mono">
                  SPECIFY HEADCOUNT
                </h2>
                <input
                  ...
                  className="w-full bg-[#0e0e0e] border border-[#333333] p-3.5 font-mono text-white focus:outline-none focus:border-white transition-all text-xs md:text-sm"
                />
                <button
                  type="submit"
                  className="w-full border border-white bg-transparent py-3.5 font-mono uppercase tracking-widest hover:bg-white hover:text-black transition-all text-xs md:text-sm font-bold cursor-pointer"
            ...
            {modalStep === "confirm" && (
              <div className="space-y-5">
                <div className="p-3.5 border border-[#333333] bg-[#0e0e0e]">
                  <p className="font-mono text-[9px] text-[#8e9192] tracking-widest uppercase mb-1">
                    SUMMARY:
                  </p>
                  <p className="text-white font-bold font-mono text-sm md:text-base">
                    {guestName || "ANONYMOUS_GUEST"}
                  </p>
                  <p className="text-white font-mono text-xs md:text-sm mt-1">
                    {rsvpType === "NO" ? "DECLINED" : `ATTENDING: ${guestCount} GUEST(S)`}
                  </p>
                </div>
                <button
                  type="button"
                  ...
                  className="w-full bg-white text-black py-3.5 font-bold font-mono uppercase tracking-widest hover:bg-[#e9c349] transition-all text-xs md:text-sm cursor-pointer disabled:opacity-50"
                >
                  {isSubmitting ? "SAVING PROTOCOL..." : "[ CONFIRM ]"}
                </button>
              </div>
            )}
          </div>
